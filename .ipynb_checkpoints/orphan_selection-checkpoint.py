...
import numpy as np
import time
import timeimport multiprocessing as mp
...
def selection(x):
    ...
    for i in range(100):
        ...
        if orpnum[i] > 0:
            ...
            np.save('/home/yunzheng/mock/orphan_new/final_selection/snapshot_%s/snap_%d.npy'%(x,i),orphan_final[Nsnap])
    ...
    np.save('/home/yunzheng/mock/orphan_new/final_selection/snapshot_%s/orphantable_final.npy'%(x),orphantable_final)
    np.savetxt('/home/yunzheng/mock/orphan_new/final_selection/snapshot_%s/final_orpnum.txt'%(x),fsnap_final)
