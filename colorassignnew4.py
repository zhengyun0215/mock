...
np.save('/home/yunzheng/mock/color/data/zcdf_new/cdf_zform.npy', cdf_zform)
print(np.max(cdf_zform))
print(np.min(cdf_zform))
...
np.save('/home/yunzheng/mock/color/data/zcdf_new/color.npy', newcolor)
...
def mul(count):
    if count % 500000 == 0:
        print('count=', count)
    invergau = inversefunc(cdf, args=(mag[count], z0), accuracy=6)
    mul_col = invergau(cdf_zform[count])
    return (mul_col)


pro = mp.Pool(processes=64)
res = pro.map(mul, range(26241654))
np.save('/home/yunzheng/mock/color/data/zcdf_new/color_mul.npy', res)
pro.close()
pro.join()
st2 = time.time()
